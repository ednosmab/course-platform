...
export const CourseService = {
  ...
  async updateCourse(
    courseId: string,
    updates: Partial<
      Pick<Course, 'title' | 'description' | 'thumbnail_url' | 'is_published'>
    >,
  ): Promise<void> {
  ...
  // ── Course Structure (modules + lessons tree) ──
  ...
  async updateModule(
    moduleId: string,
    updates: Partial<Pick<Module, 'title' | 'order_index'>>,
  ): Promise<void> {
    const { error } = await supabase
      .from('modules')
      .update(updates)
  ...
  async updateLesson(
    lessonId: string,
    updates: Partial<
      Pick<Lesson, 'title' | 'order_index' | 'is_published' | 'blocks'>
    >,
  ): Promise<void> {
    const { error } = await supabase
      .from('lessons')
  ...
  async getModuleTitle(moduleId: string): Promise<{
    title: string;
    course_id: string;
  } | null> {
