...
## 🎯 Objetivo
Garantir que a aplicação web (Admin e futura vitrine) carregue de forma instantânea, tenha excelente pontuação no Lighthouse e seja perfeitamente indexável pelos motores de busca.
...
- **`next/font`:** Use para carregar as fontes do projeto (ex: Inter). Isso evita saltos de layout (FOUT) e pré-carrega as fontes direto do servidor.
