# 🎨 SKILL: ADMIN HEADER — Padrão de Cabeçalho Compartilhado

## 🎯 Objetivo
Implementar o cabeçalho do app admin seguindo o padrão visual e comportamental definido no ADR-020.

## 📐 Referência
- **ADR-020:** `docs/adrs/ADR-020-shared-header-pattern.md`
- **Componente:** `apps/admin/src/components/AdminHeader.tsx`
...
| Root sticky | `position="sticky" top={0} zIndex={40}` |
| Root blur | `style={{ backdropFilter: 'blur(12px)' }}` |
| Nav gap | `gap={32}` |
| Nav items gap | `gap={4}` |
...
## ⚠️ Especificidades do Admin

| Aspecto | Admin | Student |
|---|---|---|
| Directive | `'use client'` | (none) |
| BrandMark | `./brand-mark` (local, next/image) | `@projeto/ui` (shared) |
| Routing | `useRouter`/`usePathname` | `onTabAction` callback |
| Nav config | `NAV_ITEMS[]` string | `NAV_TABS[].action` |
| Sticky/blur | Sim | Não |
| Mobile menu | (none) | `ScrollView horizontal` |
| `useMedia` | Não | Sim |
| Display name | `displayName` (nome completo) | `firstName` (primeiro nome) |

## 📂 Ficheiros Relacionados
- `apps/admin/src/components/AdminHeader.tsx`
- `apps/admin/src/components/brand-mark.tsx`
- `apps/admin/src/app/page.tsx`
- `apps/admin/src/app/cursos/page.tsx`
...
- [ ] BrandMark import de `./brand-mark` (next/image + priority)
- [ ] `pnpm run verify:ui` passa
- [ ] `pnpm run test` passa (92/92)
