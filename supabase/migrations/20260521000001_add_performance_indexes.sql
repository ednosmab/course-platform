...
create index concurrently if not exists idx_courses_author_id
...
create index concurrently if not exists idx_certificates_user_course
  on public.certificates(user_id, course_id);

-- Progresso: consulta de aulas concluídas por lição (partial index)
create index concurrently if not exists idx_progress_lesson_completed
